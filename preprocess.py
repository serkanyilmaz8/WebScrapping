...
# Veri setini yükleme
file_path = 'company_data.csv'  # Dosya yolunuza göre güncelleyin
data = pd.read_csv(file_path)

# '-' işaretini NaN ile değiştirip tamamen boş olan sütunları silme
...
# Temizlenmiş veri setini yeni bir CSV dosyasına kaydetme
...
# "Arz Tarihi" sütunu "Hazırlanıyor...", "Ertelendi" olmayan ve boş olmayanları filtreleme
...
# Filtrelenmiş veriyi yeni bir CSV dosyasına kaydetme
...
# Fonksiyon tanımlama: sermaye artırımı ve ortak satışı yüzdelerini bulma
...
# Fonksiyon tanımlama: fonun kullanım yerlerini analiz etme
...
# "Fonun Kullanım Yeri" sütununu silme
...
# Fonksiyon tanımlama: fiyat istikrarı gün sayısını veya 0'ı belirleme
...
# "Fiyat İstikrarı" sütununu silme
core_data.drop(columns=['Fiyat İstikrarı'], inplace=True)

# "Satmama Taahhüdü" sütununu silme
...
# Fonksiyon tanımlama: halka açıklık oranını belirleme
...
# "Halka Açıklık Oranı" sütununu silme
...
# Fonksiyon tanımlama: şehir bilgisini çıkarma
...
# Fonksiyon tanımlama: kuruluş yılını çıkarma ve şirket yaşını hesaplama
...
# Fonksiyon tanımlama: gelir ve gider verilerini ayırma ve dönüştürme
def extract_numbers(text):
    ...
    # Eğer eksik veri varsa, None ile doldur
    while len(numbers) < 6:
        numbers.append(None)
    return numbers
...
# Performans verilerini manuel olarak ekleme
...
# Arz Tarihi ve Gelir Tablosu Verileri sütunlarını silme
